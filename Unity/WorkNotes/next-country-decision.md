...
## Uygulama sözleşmesi ve ayrı Core kanıtı — 6 Eylül

Onaylanan A uygulandı: `ArmyPolicyId` (`campaign`/`budget`), `ArmyTargetTroops`, `ArmyReductionDueWeek`. `HasArmyEstablishment`, mevcut ve önerilen hedef için `GetArmyEstablishmentTerms`, `CanSetArmyEstablishment` ve `SetArmyEstablishment` public API'sidir. Campaign için hedef/vade0; aynı komut tekrarına jurnal üretmeden ret. Budget hedefi0–100000000; hedef mevcut ordunun üstündeyse asker alınmaz. Bütün fazla kuvvetin Manpower'daki yeri önceden doğrulanır, transfer sırasında sessiz kırpma yapılmaz. Yeni alım mevcut vade tarihini değiştirmez ve eski maliyetini öder.

Terms'in `campaign/scheduled/at_target/calendar` durumu, gerçek mevcut/sonraki grup/kalan fazla, tarih ve ilk düşük bütçe haftası vardır. Son grubun200'den küçük kalanı gerçektir. `CurrentArmyCost/ArmyCostAfterBatch` ve gıda karşılıkları Forecast ile aynı iki saf gider fonksiyonundan gelir. Bunlar bugünkü asker/teçhizat koşullarıdır: gelecekteki açlık, savaş kaybı veya teçhizat birikimi yüzünden değişebilir. Örneğin1200→1000 vade haftasında önce96 açlık kaybı yaşanırsa yalnız104 kişi ayrılır. Geleceğin bütün ülke gelir/gıda toplamı için tahmin yapılmaz.

Başarılı hafta sırası korunur: eski guards → ortak Dumas/ekonomi planı → eski kaynak/asker kaybı/yerel sonuçlar → dilekçe oluşumu ve dördüncü vergi hesabının kapanması → gerçek demobilizasyon → kalan orduya göre yeni Dumas duyurusu. Hafta2 içinde yeni dilekçe doğması o haftanın hak edilmiş transferini iptal etmez; bir sonraki komutu eski guard engeller. Sıfır hedefin garnizonu son ödenmiş yerel hesabında hâlâ vardır, sonraki hesapta yoktur. Yeni zorunlu olay kapısı veya game over yoktur.

Takvim sınırı: yeni vade yalnız Week+2 takvime sığıyorsa kurulur; mevcut geçerli vade korunur. Son iki haftadaki yeni olağan recruit veya sondan bir önceki haftanın son grubu fazla bırakırsa budget hedefi saklanır, due0 ve `calendar` açıklaması geçerlidir. Son hafta sonraki bütçe bulunmuyorsa `FirstReducedBudgetWeek=0`. v6 üç alanı zorunlu doğrular; eski v1–5 açıkça campaign'e taşınır. Accord için3, victory için4 ve Dumas için5 eşikleri bağımsız kaldı.

Root çalıştırdığı `output/core-probes/ArmyEstablishmentProbe-2026-09-06T02-50-39-560Z-52837607/probe.stdout.log` dosyasında **PASS checks=148** verdi; dosya salt okunur incelendi. `Unity/WorkNotes/ArmyEstablishmentProbe.cs` yalnız public komutlar kullanır.1200→1000 rotası iki eski bütçeyi, hafta2 dilekçesiyle beraber200 Manpower'ı, sonra136→120/40→34 gideri, ücretli geri alımı ve iptali doğrular. Ayrı rota12 başarılı haftada6 grup ile0 askere/3600 Manpower'a varır; hafta13 devam eder ve normal recruit ile toparlanır. Gerçek log hafta12'de53 altın/7 asker gıdası öderken transferden sonraki hafta13'te0/0 gösterir. Bu **pure Core** sonucudur; Unity veya player kanıtı olarak sunulmaz. Bu agent hiçbir derleme/test/probe/player çalıştırmadı; root ortak kapıyı yönetir.
