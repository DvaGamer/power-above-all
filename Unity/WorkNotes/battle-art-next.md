...
## C uygulaması — kuru geçit, kaynak freeze

- Root C'yi ayrı dar paket olarak yetkilendirdi. Önceki A/B/C karşılaştırması esas alındı; son `hill-natural-victory-20260905-234734-753-1141ea63/shots/05-contested-crossing.png` yeniden görülerek kare toprak parçasının özellikle hedef halkası içinde sert durduğu doğrulandı.
- `BuildLandscape` içindeki tek `Crossing dirt` küp çağrısı `BuildDryFord()` ile değiştirildi. Yeni özel metot, mevcut toprak malzemesiyle yolun iki yönde kıyıya doğru genişlediği tek düz yüzey kurar. Kum/çakıl, köprü, engel, yeni doku veya materyal eklenmedi; yol ve geçit aynı sıcak toprağı paylaşır.
- Kesin yatay sınırlar: bütün `4 ≤ x ≤ 8` aralığında kuru yüzey tam `−3 ≤ z ≤ 5` bandını kaplar. Dolayısıyla suyun içinden geçen dar sahte patika veya eksik kuru kenar yoktur. Yeni yüzeyin bu kanal aralığında `z < −3` veya `z > 5` kısmı yoktur. Küçük kıyı genişlemeleri yalnız kanalın dışında; tüm mesh sınırı `x = −.5..12.5`, `z = −3..5.16`. En kuzeydeki `5.16` değeri yalnız doğu kıyısında `x = 8.55` noktasındadır.
- Doğu/batı uçları mevcut eğik yolun genişliğine oturur. On iki kesit birbirine iki üçgenle bağlanır: 24 köşe, 22 üçgen. Kesit x değerleri `−.5, .9, 2.1, 3.35, 4, 5.25, 6.75, 8, 8.55, 9.75, 11.1, 12.5`; iç kanal kesitlerinde iki z kenarı da sabittir. Dış kıyıların farklı açılarla açılması eski bağımsız dikdörtgen plaka hissini azaltır.
- Yüzey yüksekliği her köşede `.065`: eski küpün üst yüzeyiyle aynıdır. Geçit kendi gölgesini atmaz; sahneye yükseltilmiş bir engel algısı eklenmez. Mesh mevcut `meshes` listesine kayıtlı ve `world` altındadır; aynı savaş temizliğiyle kaldırılır. Collider yoktur.
- `TerrainHeight`, `InCreek`, nehir meshleri/uçları/su yüksekliği, tepe boyaması, yol, RNG, alaylar/konumlar, girdi/API/simülasyon, ışık, kamera ve hedef halkası değiştirilmedi. Yetki gereği Git komutu da çalıştırılmadı.
- Kaynaktaki gerçek üç sayı dizisi okunarak salt bellek geometrisi denetlendi: 81 kanal kesitinde tam kuru bant, artan x kesitleri, pozitif genişlik ve 22 yukarı bakan üçgen PASS. Unity, player, derleme veya commit yok. Görsel kabul henüz yok; root taze doğal savaş ve aynı başlangıç/yoğun temas kareleriyle değerlendirecek.

## C gerçek kare kabulü

- Önce `hill-natural-victory-20260905-234734-753-1141ea63/shots/02-deployment-paused.png`, sonra `accord-ford-natural-20260906-002505-474-25aa327c/shots/02-deployment-paused.png` ve `05-contested-crossing.png` gerçekten açıldı (`output/verify/` altında).
- Eski ayrı dikdörtgen plaka kaybolmuş; aynı toprak rengiyle yol iki kıyıda kuru geçide genişliyor. Şekil organik ve düz okunuyor, köprü/taş engeli gibi görünmüyor. Dere iki tarafta belirgin; ortasından suyu yaran dar bir sahte patika yok. Hedef halkası, arabalar ve birlikler ayırt ediliyor; yoğun temas karesinde yeni bir örtme sorunu görülmedi.
- Dar C değişikliği görsel olarak kabul edilebilir; yeni gölge, boya veya kıyı ayrıntısı gerekmiyor. Su kollarının genel düz görünümü bu küçük paketin çözmediği bir sınır olarak kalır; bunun için otomatik yeni iş açılmadı.
- Root bu yeniden kullanılan doğal savaş koşusunu doğru biçimde PARTIAL/native exit0, 9 PNG/15 assertion/8 JSON olarak bildirdi; sonuç aynı `125.803 s / 196` kayıp ve `+24` malzeme. Görsel inceleme bu koşuyu taze build kapısı diye yeniden adlandırmaz. Bu ajan Unity/oyuncu veya Git çalıştırmadı; Assets freeze korundu.
